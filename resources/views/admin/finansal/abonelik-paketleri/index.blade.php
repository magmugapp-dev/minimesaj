...
@section('baslik', 'Abonelik Paketleri')

@section('icerik')
    <div class="space-y-6 p-6">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                <h2 class="text-xl font-semibold text-gray-900">Premium abonelik paketleri</h2>
                <p class="text-sm text-gray-500">iOS ve Android urun kodlarini buradan yonetebilirsin.</p>
            </div>
            <a href="{{ route('admin.finansal.abonelik-paketleri.create') }}"
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-gray-200 text-left text-xs uppercase tracking-wider text-gray-500">
                            ...
                            <th class="px-3 py-3">Store urunleri</th>
                            <th class="px-3 py-3">Sure</th>
                        ...
                        @forelse ($paketler as $paket)
                            <tr class="hover:bg-gray-50">
                                <td class="px-3 py-3">
                                    ...
                                    <p class="text-xs text-gray-400">Sira: {{ $paket->sira }}</p>
                                </td>
                                <td class="px-3 py-3">
                                    <p class="text-gray-700">Android: {{ $paket->android_urun_kodu ?: '-' }}</p>
                                    <p class="text-gray-500">iOS: {{ $paket->ios_urun_kodu ?: '-' }}</p>
                                </td>
                                <td class="px-3 py-3 font-semibold text-gray-900">{{ $paket->sure_ay }} ay</td>
                                <td class="px-3 py-3">
                                ...
                                <td class="px-3 py-3">
                                    <div class="flex justify-end gap-2">
                                        <a href="{{ route('admin.finansal.abonelik-paketleri.edit', $paket) }}"
                                            class="rounded-lg bg-indigo-50 px-3 py-1.5 text-xs font-medium text-indigo-700 hover:bg-indigo-100">
                                            Duzenle
                                        </a>
                                        <form method="POST"
                                            action="{{ route('admin.finansal.abonelik-paketleri.destroy', $paket) }}"
                                            onsubmit="return confirm('Bu abonelik paketini silmek istediginize emin misiniz?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-3 py-10 text-center text-sm text-gray-500">
                                    Henuz abonelik paketi tanimlanmadi.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
